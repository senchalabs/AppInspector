    ...
    var Boot = {
        ...
        init: function () {
            var scriptEls = doc.getElementsByTagName('script'),
                n = scriptEls.length,
                re = /ext(\-[a-z\-]+)?\.js$/,
                entry, script, src, state, baseUrl, key;
            ...
            if (!baseUrl) {
                baseUrl = scriptEls[scriptEls.length - 1].src;
            }
        ...
        loadSync: function (request) {
            _syncMode++;
            var request = request.url ? request : {url: request},
                url = request.url,
                urls = url.charAt ? [ url ] : url,
                length = urls.length,
                buster = _config.disableCaching &&
                    ('?' + _config.disableCachingParam + '=' + (+new Date())),
                content, entry, i, key, status, xhr, exception;
            ...
            for (i = 0; i < length; ++i) {
                ...
                // Make the request using synchronous Ajax:
                //
                exception = false;
        ...
        watch: function (entry) {
             ...
             */
            if (('addEventListener' in el) || !('readyState' in el)) {
                el.onload = onLoadFn;
            } else {
